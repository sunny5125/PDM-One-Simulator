...
/**
 * <P>
 * Overlay grid of the world where each node is put on a cell depending
...
public class ConnectivityGrid {
	 ...
	 * @param worldSizeX Width of the world (biggest possible x coordinate)
	 * @param worldSizeY Height of the world (biggest possible y coordinate)
	...
	/**
	 * Connects a node to all nodes that are in the same cell or in
	 * adjacent cells. Good algorithm for small (less than 2k) amounts
	 * of nodes or highly clustered nodes. For larger amounts of nodes
	 * connectAllNearNodes() could be a better choice.
	 ...
	 * Good algorithm for large amount of nodes (more than 2k) that are
	 * relatively evenly distributed, lousy for small amounts or highly
	 * clustered nodes. For smaller amounts, use connectToNearNodes()
	 ...
	 * @param node Node to connect
	 * @param cells Cell array where the other nodes are looked from
	 */
	...
	/**
	 * A single cell in the cell grid. Contains the nodes that are 
	...
	public class GridCell {
		 ...
		 * @param to The cell where the node should be moved to
		 */
