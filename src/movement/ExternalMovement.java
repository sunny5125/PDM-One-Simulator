...
public class ExternalMovement extends MovementModel {
	...
	static {
		DTNSim.registerForReset("movement.ExternalMovement");
		reset();
	}
	...
	/** 
	 * Copyconstructor. Gives out location data for the new node from 
	 * location queue.
	...
	private ExternalMovement(MovementModel mm) {
		...
		if (initLocations.size() > 0) { // we have location data left
			...
			this.intialLocation = latestLocation = initLoc.getValue();
