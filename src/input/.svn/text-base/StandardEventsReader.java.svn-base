 ...
 * </TT>
 * </P><P>
 * All actions must have first four fields. SEND, DELIVERED and ABORT actions
 * need host2Id field too (the host who the message is/was being 
 * transferred to). CREATE action needs the additional size 
...
public class StandardEventsReader implements ExternalEventsReader {
	...
	/**
	 * Parses a host address from a hostId string (the numeric part before
	...
	private int getHostAddress(String hostId) {
		...
		if (hostId.matches("^\\d+$")) {
			...
			addressPart = parts[parts.length-1]; // last occurrence is the addr
		}
		else {
